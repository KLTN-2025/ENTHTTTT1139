...
export const ProgressService = {
  ...
  async checkCanProceed(
    currentLectureId: string,
    nextLectureId: string
  ): Promise<{ canProceed: boolean; message: string; currentProgress?: ProgressCheckResponse }> {
    const token = localStorage.getItem('accessToken');
    if (!token) {
      // throw new Error('Không tìm thấy token xác thực');
    }
    const response = await axiosInstance.get(
      `/progress/can-proceed/${currentLectureId}/${nextLectureId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  async checkCurriculumAccess(
    data: CurriculumAccessCheckDto
  ): Promise<{ canProceed: boolean; message: string; currentProgress?: ProgressCheckResponse }> {
    const token = localStorage.getItem('accessToken');
    if (!token) {
      throw new Error('Không tìm thấy token xác thực');
