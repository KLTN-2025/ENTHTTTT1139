...
export const LearningDashboard: React.FC<LearningDashboardProps> = ({ courses }) => {
  const { calculateCourseProgress } = useProgressTracking();

  // Phân loại khóa học theo trạng thái
  const { inProgressCourses, completedCourses, notStartedCourses, courseStats } = useMemo(() => {
    if (!courses?.length) {
      return {
        inProgressCourses: [],
        completedCourses: [],
    ...
    inProgress.sort(
      (a, b) => calculateCourseProgress(b.moduleIds) - calculateCourseProgress(a.moduleIds)
    );
          ...
          <ProgressBar
            ...
            color={
              courseStats.overallProgress === 100
                ? 'success'
                : courseStats.overallProgress >= 70
                  ? 'primary'
                  : courseStats.overallProgress >= 30
                    ? 'warning'
                    : 'danger'
            }
