...
import { ChevronLeft, ChevronRight, CheckCircle2, PlayCircle, Video } from 'lucide-react';
                    ...
                    title={
                      lectureCompletion.canProceed
                        ...
                        : lectureCompletion.completionRatio > 0
                          ? `Đang học - ${Math.round(lectureCompletion.completionRatio * 100)}% hoàn thành`
                          : 'Chưa bắt đầu học'
                    }
                  >
                    {lectureCompletion.canProceed ? (
                ...
                {isLoggedIn && lectureCompletion.completionRatio > 0 && (
                  <div className="flex items-center gap-1 text-sm">
                    <div className="px-2 py-0.5 bg-gray-100 rounded-full text-sm flex items-center">
                      <span
                        className={`text-xs ${lectureCompletion.canProceed ? 'text-green-600' : 'text-yellow-600'}`}
                      >
                        {lectureCompletion.canProceed
                          ? 'Đã hoàn thành'
                ...
                <div className="flex items-center">
                  <div className="px-3 py-1 bg-gray-100 rounded-full text-sm flex items-center">
                    ...
                    <span className="text-gray-600 text-xs">
                      {lectureCompletion.canProceed
                        ? 'Đã hoàn thành'
                        : `${Math.round(lectureCompletion.completionRatio * 100)}%`}
                    </span>
                  </div>
                </div>
              )}
