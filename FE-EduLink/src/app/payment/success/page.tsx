  ...
  useEffect(() => {
    ...
    const capturePayment = async () => {
      try {
        ...
        if (
          (responseData.data && responseData.data.success) ||
      ...
      <Card className="p-8 max-w-3xl mx-auto text-center">
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-6">
          <p className="text-gray-600 mb-2">
            Mã giao dịch: <span className="font-medium">{paymentDetails?.id}</span>
          </p>
          <p className="text-gray-600 mb-2">
            Trạng thái: <span className="font-medium">{paymentDetails?.status}</span>
          </p>
          {paymentDetails?.payer?.email_address && (
            <p className="text-gray-600 mb-2">
              Email: <span className="font-medium">{paymentDetails.payer.email_address}</span>
            </p>
          )}
        </div>
        ...
        <p className="text-gray-600 mb-6">
          ...
          <br />
          Bạn sẽ được chuyển hướng đến trang khóa học trong 5 giây.
        </p>
