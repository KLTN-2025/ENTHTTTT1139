...
export class PaypalService {
  ...
  private initPaypalConfig() {
    const mode = this.configService.get<'sandbox' | 'live'>(
      'PAYPAL_MODE',
      'sandbox',
    );
    ...
    this.adminPaypalEmail = this.configService.get<string>(
      'PAYPAL_ADMIN_EMAIL',
      '',
    );
    this.exchangeRateApiKey = this.configService.get<string>(
    ...
    if (!this.clientId || !this.clientSecret) {
      this.logger.error(
        'Thiếu thông tin xác thực PayPal. Vui lòng kiểm tra biến môi trường.',
      );
    }
  ...
  async convertVNDtoUSD(amountVND: number): Promise<number> {
    try {
      ...
      if (
        ...
        this.logger.error(
          `Lỗi khi chuyển đổi tiền tệ: ${JSON.stringify(response.data)}`,
        );
      ...
      if (response.data && response.data.result) {
        ...
        this.logger.warn(
          `Không thể xác định kết quả từ API, sử dụng tỷ giá ước tính: ${amountUSD} USD`,
        );
      }

      this.logger.log(
        `Chuyển đổi thành công: ${amountVND} VND = ${amountUSD} USD`,
      );
  ...
  async createCustomerPayment(
    ...
    try {
      ...
      if (!this.adminPaypalEmail) {
        this.logger.error(
          'Thiếu email PayPal người nhận thanh toán. Vui lòng kiểm tra biến môi trường PAYPAL_ADMIN_EMAIL.',
        );
      ...
      if (totalAmount <= 0) {
        this.logger.error(
          `Tổng giá trị thanh toán không hợp lệ: ${totalAmount}`,
        );
      ...
      const totalUSD = convertedItems.reduce(
      ...
      this.logger.log(
        `Tỷ giá chuyển đổi trung bình: 1 USD = ${exchangeRate.toFixed(2)} VND`,
      );
      ...
      this.logger.log(
        `Tổng giá trị đơn hàng sau khi tính lại: ${formattedTotal} USD (từ ${totalAmount} VND)`,
      );
      ...
      const requestPayload = {
        ...
        application_context: {
          ...
          brand_name: 'EduLink Learning Platform',
      ...
      this.logger.log(
        `Gửi yêu cầu tạo thanh toán đến PayPal: ${JSON.stringify(requestPayload)}`,
      );
      ...
      if (!approvalLink) {
        this.logger.error(
          'Không tìm thấy link phê duyệt trong phản hồi từ PayPal',
        );
        throw new Error(
          'Không tìm thấy link phê duyệt trong phản hồi từ PayPal',
        );
      ...
      this.logger.error(
        `Lỗi khi tạo thanh toán PayPal cho khách hàng: ${error.message}`,
      );
      ...
      if (error.response) {
        ...
        this.logger.error(
          `Chi tiết lỗi từ PayPal [${statusCode}]: ${JSON.stringify(responseData)}`,
        );

        // Xử lý các mã lỗi phổ biến
        if (statusCode === 401) {
          throw new Error(
            'Lỗi xác thực với PayPal, vui lòng kiểm tra cấu hình',
          );
    ...
    try {
      ...
      const payoutRequest = {
        sender_batch_header: {
          ...
          email_message:
            'Bạn đã nhận được thanh toán cho khóa học của mình trên nền tảng của chúng tôi.',
        },
        items: [
          {
  ...
  async capturePayment(paymentId: string): Promise<any> {
    try {
      ...
      this.logger.error(
        `Lỗi khi xác nhận thanh toán PayPal: ${error.message}`,
        error.stack,
      );
      ...
      if (error.response && error.response.data) {
        this.logger.error(
          `Chi tiết lỗi từ PayPal: ${JSON.stringify(error.response.data)}`,
        );
